...
export async function addTransaction(tx: Omit<Transaction, 'id'>, accountId?: number): Promise<Result<string, DbError>> {
  try {
    ...
    return err(new DbError(`addTransaction failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
...
export async function upsertTransaction(tx: Omit<Transaction, 'id'>, accountId?: number): Promise<Result<string, DbError>> {
  try {
    ...
    return err(new DbError(`upsertTransaction failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function updateTransactionAmount(txid: string, amount: number, accountId?: number): Promise<Result<void, DbError>> {
  try {
    const database = getDatabase()
    const accId = accountId ?? 1
    ...
    return err(new DbError(`updateTransactionAmount failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function getTransactionsByLabel(label: string, accountId?: number): Promise<Result<Transaction[], DbError>> {
  try {
    const database = getDatabase()
    ...
    return err(new DbError(`getTransactionsByLabel failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
...
export async function updateTransactionStatus(
  ...
  try {
    ...
    return err(new DbError(`updateTransactionStatus failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
...
export async function updateTransactionLabels(
  ...
  try {
    ...
    if (rows.length === 0) return ok(undefined) // txid doesn't belong to this account — no-op
    ...
    return err(new DbError(`updateTransactionLabels failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function getAllLabels(accountId?: number): Promise<Result<string[], DbError>> {
  try {
    const database = getDatabase()
    ...
    return ok(rows.map(row => row.label))
  } catch (e) {
    return err(new DbError(`getAllLabels failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function getTopLabels(limit = 3, accountId?: number): Promise<Result<string[], DbError>> {
  try {
    const database = getDatabase()
    ...
    return err(new DbError(`getTopLabels failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function getTransactionLabels(txid: string, accountId?: number): Promise<Result<string[], DbError>> {
  try {
    const database = getDatabase()
    ...
    return err(new DbError(`getTransactionLabels failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function getTransactionByTxid(txid: string, accountId: number): Promise<Result<Transaction | null, DbError>> {
  try {
    const database = getDatabase()
    ...
    return err(new DbError(`getTransactionByTxid failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
...
export async function searchTransactionsByLabels(
  ...
  try {
    ...
    return err(new DbError(`searchTransactionsByLabels failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
...
export async function searchTransactions(
  ...
  try {
    ...
    return err(new DbError(`searchTransactions failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function getPendingTransactionTxids(accountId?: number): Promise<Result<Set<string>, DbError>> {
  try {
    const database = getDatabase()
    ...
    return ok(new Set(rows.map(r => r.txid)))
  } catch (e) {
    return err(new DbError(`getPendingTransactionTxids failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function getKnownTxids(accountId?: number): Promise<Result<Set<string>, DbError>> {
  try {
    const database = getDatabase()
    ...
    return err(new DbError(`getKnownTxids failed: ${e instanceof Error ? e.message : String(e)}`, 'QUERY_FAILED', e))
  }
}
 ...
 */
export async function deleteTransactionsForAccount(accountId: number): Promise<Result<number, DbError>> {
  try {
    const database = getDatabase()
