...
async function backfillNullAmounts(
  ...
  try {
    const allTxsResult = await getAllTransactions(accountId, { limit: 100000 })
    if (!allTxsResult.ok) {
...
export async function syncWallet(
  ...
  try {
    ...
    // not sign anything). Derived-key re-derivation happens only in the browser
    // send fallback. We intentionally omit it here to avoid routing key
...
export async function needsInitialSync(addresses: string[], accountId?: number): Promise<boolean> {
  ...
  if (accountId !== undefined) {
    const txCountResult = await getTransactionCount(accountId)
    const hasTxs = txCountResult.ok && txCountResult.value > 0
    if (!hasTxs) {
